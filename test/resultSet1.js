...
describe('12. resultSet1.js', function() {
  ...
  var createTable =
      "BEGIN \
          ...
          BEGIN \
              EXECUTE IMMEDIATE ('DROP TABLE nodb_rs_emp'); \
          EXCEPTION \
              WHEN e_table_missing \
              THEN NULL; \
          END; \
          EXECUTE IMMEDIATE (' \
              CREATE TABLE nodb_rs_emp ( \
                  employees_id NUMBER,  \
      ...
      "DECLARE \
       ...
       BEGIN \
          FOR i IN 1..217 LOOP \
             x := x + 1; \
             n := 'staff ' || x; \
             INSERT INTO nodb_rs_emp VALUES (x, n); \
          END LOOP; \
       END; ";
  var rowsAmount = 217;
  ...
  describe('12.1 Testing resultSet option', function() {
    it('12.1.1 when resultSet option = false, content of result is correct', function(done) {
      ...
      connection.execute(
        "SELECT employees_name FROM nodb_rs_emp",
        [],
  ...
  describe('12.3 Testing function getRows()', function() {
    ...
    it('12.3.8 query 0 row', function(done) {
      ...
      var accessCount = 0;
      connection.execute(
        "SELECT employees_name FROM nodb_rs_emp WHERE employees_id > 300",
        [],
        { resultSet: true },
        function(err, result) {
          should.not.exist(err);
  ...
  describe('12.7 Testing maxRows', function() {
    it('12.7.1 maxRows option is ignored when resultSet option is true', function(done) {
      ...
      var rowsLimit = 50;
      connection.execute(
        "SELECT * FROM nodb_rs_emp ORDER BY employees_id",
        [],
