...
function _initCLib(options) {
  ...
  if (options.binaryLocation !== undefined) {
    binaryLocations.splice(0, 0, options.binaryLocation + '/' + nodbUtil.BINARY_FILE,
      options.binaryLocation + '/' + nodbUtil.BUILD_FILE);
  }
  let oracledbCLib;
...
async function _verifyOptions(options, inCreatePool) {
  ...
  if (options.accessToken !== undefined) {
    ...
    if (typeof options.accessToken === 'function') {
      ...
      accessToken = await options.accessToken(false);
      if (!nodbUtil.isTokenValid(accessToken)) {
        accessToken = await options.accessToken(true);
      }
    } else {
    ...
    if (accessToken.privateKey !== undefined) {
      ...
      accessToken.privateKey = accessToken.privateKey.replace(/\n/g, '');
      accessToken.privateKey = accessToken.privateKey.replace('-----BEGIN PRIVATE KEY-----', '');
      accessToken.privateKey = accessToken.privateKey.replace('-----END PRIVATE KEY-----', '');
    }
