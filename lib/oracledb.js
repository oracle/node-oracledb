...
//modify the get connection to provide an extended connection object
var getConnectionOrg = oracledb_ins.getConnection;
oracledb_ins.getConnection = function () {
  var callback = arguments[arguments.length - 1];
  arguments[arguments.length - 1] = Connection.wrapOnConnection(callback);

  getConnectionOrg.apply(this, arguments);
};

//modify the pool to enable to provide extend connection object
var createPoolOrg = oracledb_ins.createPool;
oracledb_ins.createPool = function () {
  var callback = arguments[arguments.length - 1];

  arguments[arguments.length - 1] = function onPoolCreated(error, pool) {
    if (error) {
      callback(error);
    } else {
      var getPooledConnection = pool.getConnection;

      pool.getConnection = function (poolCallback) {
        getPooledConnection.call(pool, function (connectionError, connection) {
          if (connectionError) {
            poolCallback(connectionError);
          } else {
            Connection.modify(connection);
            poolCallback(connectionError, connection);
          }
        });
      };
  ...
  createPoolOrg.apply(this, arguments);
};
