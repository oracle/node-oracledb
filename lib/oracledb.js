...
async function _verifyOptions(options, inCreatePool) {
  ...
  // check connection creation specific options
    ...
    // privilege must be an integer >= 0
    if (options.privilege !== undefined) {
      errors.assertParamPropValue(Number.isInteger(options.privilege) &&
        options.privilege >= 0, 1, "privilege");
  ...
  if (options.accessToken !== undefined) {
    ...
    if (inCreatePool) {
      ...
      errors.assert(options.accessTokenCallback === undefined ||
        typeof options.accessToken !== 'function', errors.ERR_TOKEN_CALLBACK_DUP);
    } else {
    ...
    if (typeof options.accessToken === 'function') {
      outOptions.accessTokenCallback = options.accessToken;
