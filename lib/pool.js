...
function completeConnectionRequest(config, getConnectionCb) {
  ...
  self._getConnection(config, function(err, connInst, actualTag, newSession) {
    if (err) {
    ...
    if (typeof self.sessionCallback === 'function' &&
        (newSession || actualTag != config.tag)) {
      self.sessionCallback(connInst, config.tag, actualTag,
        function(err) {
          if (err) {
