  ...
  _verifyMessage(message) {
    ...
    if (this._isJson) {
      ...
      if (!(payload instanceof BaseDbObject) ||
        (this._payloadTypeName !== payload._impl._objType.name))
    ...
    if (message.correlation !== undefined) {
      errors.assertPropLength(message.correlation, "string", 128,
        "correlation");
    }
    ...
    if (message.exceptionQueue !== undefined) {
      errors.assertPropLength(message.exceptionQueue, "string", 128,
        "exceptionQueue");
    }
    ...
    if (message.recipients !== undefined) {
      // Check recipients length and type
      errors.assertPropLength(message.recipients, "array", 1024, "recipients");
      // Check value and length of each recipient
      message.recipients.forEach((recipient, index) => {
        errors.assertPropLength(recipient, "string", 128,
          `recipient at index ${index}`);
      });
    }
