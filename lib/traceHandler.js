...
/**
 * TraceHandler interface definition which application needs to implement
...
class TraceHandlerBase {
   ...
   * Useful for tracking pool growth or initialization activity.
   */
  onPoolExpand(/*pool*/) {}

  /**
   * Called when the pool shrinks and a connection is removed,
   * such as during timeouts or pool resizing. Useful for
   * monitoring resource cleanup or shrink events.
   */
  onPoolShrink(/*pool*/) {}

  /**
   * Called when a connection is successfully acquired from the pool.
   * Useful for tracking active usage or checkout activity.
   */
  onPoolAcquire(/*pool*/) {}

  /**
   * Called when a connection is released back to the pool.
   * Useful for monitoring returned or idle connections.
   */
  onPoolRelease(/*pool*/) {}

  /**
   * Called when a connection request is queued because the pool
   * cannot grow further. Useful for monitoring queued requests.
   */
  onPoolWait(/*pool*/) {}

  /**
   * Called when a queued connection request times out before it
   * can be served. Useful for tracking request timeouts or failures.
   */
  onPoolRequestTimeout(/*pool*/) {}

  /**
   * Called when a requested connection is obtained from the pool's
   * free list (cache hit). Useful for analyzing connection reuse.
   */
  onPoolConnectionHit(/*pool*/) {}

  /**
   * Called when a connection request requires creating a new
   * connection (cache miss). Useful for monitoring connection creation.
   */
  onPoolConnectionMiss(/*pool*/) {}

  /**
   * Called when the pool is closed. Useful for tracking shutdown
   * events or resetting counters.
   */
  onPoolClose(/*pool*/) {}
}
