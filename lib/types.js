...
class DbType {
  ...
  constructor(num, name, columnTypeName, options) {
    ...
    this._is_fast = options.is_fast || true;
    ...
    if (this._oraTypeNum === 8 && this._csfrm === constants.CSFRM_IMPLICIT) {
      dbTypeByOraTypeNum.set(this._oraTypeNum, this);
    }
...
const DB_TYPE_BFILE = new DbType(2020,
  "DB_TYPE_BFILE", "BFILE",
  { oraTypeNum: 114, bufferSizeFactor: 4000, is_fast: false });
...
const DB_TYPE_BLOB = new DbType(2019,
  "DB_TYPE_BLOB", "BLOB",
  { oraTypeNum: 113, bufferSizeFactor: 112, is_fast: false });
...
const DB_TYPE_CURSOR = new DbType(2021,
  ...
  { oraTypeNum: 102, bufferSizeFactor: 4, is_fast: false });
...
const DB_TYPE_LONG_RAW = new DbType(2025,
  ...
  { oraTypeNum: 24, bufferSizeFactor: 2 ** 31 - 1, is_fast: false });
...
const DB_TYPE_ROWID = new DbType(2005,
  "DB_TYPE_ROWID", "ROWID",
  { oraTypeNum: 11, bufferSizeFactor: 18, is_fast: false });
...
const DB_TYPE_TIMESTAMP_TZ = new DbType(2013,
  ...
  { oraTypeNum: 181, bufferSizeFactor: 13, is_fast: false });
const DB_TYPE_UROWID = new DbType(2030,
  "DB_TYPE_UROWID", "UROWID",
  { oraTypeNum: 208, is_fast: false });
...
const DB_TYPE_XMLTYPE = new DbType(2032,
  ...
  { oraTypeNum: 109, csfrm: constants.CSFRM_IMPLICIT,
    bufferSizeFactor: 2147483647, is_fast: false });
const DB_TYPE_VECTOR = new DbType(2033,
  "DB_TYPE_VECTOR", "VECTOR",
  { oraTypeNum: 127, is_fast: false });
...
class IntervalYM {
  constructor(obj) {
    ...
    this.months = (obj?.months || 0) % 12;
    this.years = (obj?.years || 0) + Math.trunc((obj?.months || 0) / 12);
  }
}
...
class IntervalDS {
  constructor(obj) {
    ...
    this.seconds = (obj?.seconds || 0) % 60;
    const totalMinutes = (obj?.minutes || 0) +
      Math.trunc((obj?.seconds || 0) / 60);
    this.minutes = totalMinutes % 60;
    const totalHours = (obj?.hours || 0) + Math.trunc(totalMinutes / 60);
    this.hours = totalHours % 24;
    this.days = (obj?.days || 0) + Math.trunc(totalHours / 24);
  }
}
