...
function queryStream(sql, binding, options) {
  ...
  var streamNumRows = options.streamNumRows || self._oracledb.streamNumRows || 100;
  ...
  self._execute(sql, binding, options, function onExecuteDone(err, result) {
    ...
    stream.emit('metadata', result.resultSet.metaData);
    ...
    if (stream.closed) {
      ...
      stream.nextRow = function fetchNextRow(streamCallback) {
        if (readRows && readRows.length) {
          ...
          result.resultSet.getRows(streamNumRows, function onRow(rowError, rows) {
            if (rowError) {
