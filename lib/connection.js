...
async function execute(sql, a2, a3) {
  ...
  if (result.resultSet) {
    ...
    if (!executeOpts.resultSet) {
      result.rows = await result.resultSet._getAllRows(executeOpts);
      delete result.resultSet;
    }
  }

  // process out binds; ensure all result sets returned in out binds have their
  // fetch array size fixed
  if (result.outBinds) {
    for (const key in result.outBinds) {
      const value = result.outBinds[key];
      if (value instanceof this._oracledb.ResultSet) {
        value._setup(executeOpts);
      }
    }
  }
  ...
  if (result.implicitResults) {
    for (const key in result.implicitResults) {
      ...
      implicitResult._setup(executeOpts);
      if (!executeOpts.resultSet) {
