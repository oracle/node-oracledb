...
class Connection extends EventEmitter {
  ...
  async _processBindValue(bindInfo, value, iterNum, allowArray) {
    ...
    // handle binding strings
      ...
      if (bindInfo.type === constants.DB_TYPE_NCHAR ||
          bindInfo.type === constants.DB_TYPE_NVARCHAR ||
          bindInfo.type === constants.DB_TYPE_NCLOB) {
        value = Buffer.from(value, this._impl._ncharEncoding);
        if (this._impl._ncharEncoding === constants.ENCODING_UTF16)
          value = value.swap16();
      } else {
    ...
    // handle binding numbers
      ...
      bindInfo.values[iterNum] = value;
      if (Number.isNaN(value) && bindInfo.type === constants.DB_TYPE_NUMBER) {
