...
const APP_CONTEXT_KEY_MAX_BYTES = 128;
const APP_CONTEXT_VALUE_MAX_BYTES = 4 * 1000;
...
// converters are invoked when retrieving DBObject values.
//---------------------------------------------------------------------------
  ...
  if (options.dbObjectTypeHandler) {
    ...
    if (result !== undefined) {
      ...
      if ([types.DB_TYPE_CLOB, types.DB_TYPE_NCLOB, types.DB_TYPE_BLOB,
        ...
        // converters for LOBs are not supported.
        return errors.throwNotImplemented('DbObjConverter for LOBs');
      }
...
class Connection extends EventEmitter {
  ...
  async _processBindUnit(bindInfo, bindUnit, inExecuteMany) {
    ...
    if (bindUnit.type !== undefined) {
      if (typeof bindUnit.type === 'string') {
        if (this._impl.pipelineMode) {
          errors.throwErr(errors.ERR_PIPELINE_DBOBJECTS_NOT_SUPPORTED);
        }
  ...
  async close(a1) {
    ...
    try {
      ...
      if (this._pool) {
        ...
        traceHandler.getTraceInstance()?.onPoolRelease(this._pool);
      } else {
  ...
  //---------------------------------------------------------------------------
  async commit() {
    await this._commit.apply(this, arguments);
  }
  ...
  setEndUserSecurityContext(context) {
    ...
    if (typeof this.protocol !== 'string' || this.protocol !== 'TCPS') {
      errors.throwErr(errors.ERR_END_USER_SECURITY_CONTEXT_REQUIRES_TCPS);
    }

    errors.assertParamValue(
      context instanceof EndUserSecurityContext,
      1
    );

    this._impl.setEndUserSecurityContext(context);
  }

  clearEndUserSecurityContext() {
    errors.assertArgCount(arguments, 0, 0);
    errors.assert(this._impl, errors.ERR_INVALID_CONNECTION);
    this._impl.clearEndUserSecurityContext();
  }
  ...
  async _processExecuteArgs(sql, a2, a3) {
    const numIters = 1;
    let binds = [];
    let options = {};
    ...
    return {sql, numIters, binds, options, executeManyFlag: false};
  }

  async _execute() {

    // process arguments
    const { sql, numIters, binds, options, executeManyFlag } =
    await this._processExecuteArgs.apply(this, arguments);
    ...
    try {
      ...
      result = await this._impl.execute(sql, numIters, binds, options,
        executeManyFlag);
    } catch (err) {
    ...
    return await this._transformExecuteResult(result, options);
  }
  ...
  //---------------------------------------------------------------------------
  async execute() {
    return await this._execute.apply(this, arguments);
  }

  async _handleFetchForExecuteInPipeline(result, options) {
    if (result.warning) {
      result.warning = errors.transformErr(result.warning);
    }

    if (result.resultSet !== undefined) {
      // just close resultSet for pipelineOp.opType 'execute'
      // To fetch rows, 'fetch*' used instead in pipeline mode
      await result.resultSet.close();
      delete result.resultSet;
    }

    this._processOutBinds(result, options);
    await this._processImplicitResults(result, options);
  }

  async _handleFetchInPipeline(result, resultSet, fetchOpPipeline) {
    let shouldClose = false;
    try {
      if (fetchOpPipeline.opType === 'fetchOne') {
        const row = await resultSet._getRow();
        result.rows = row ? [row] : [];
        shouldClose = true;
      } else if (fetchOpPipeline.opType === 'fetchMany') {
        if (fetchOpPipeline.numRows === 0)
          result.rows = [];
        else {
          const maxRows = resultSet._impl.maxRows;
          const rowsToFetch = (maxRows > 0) ?
            Math.min(fetchOpPipeline.numRows, maxRows) :
            fetchOpPipeline.numRows;
          result.rows = await resultSet._doGetRows(rowsToFetch);
        }
        shouldClose = true;
      } else
      ...
      if (shouldClose)
        await resultSet._close();
      delete result.resultSet;
    }
  }

  _processOutBinds(result, options) {
  ...
  async _processImplicitResults(result, options, fetchOpPipeline) {
    if (result.implicitResults) {
      if (fetchOpPipeline) {
        delete result.implicitResults;
        return;
      }
  ...
  async _transformExecuteResult(result, options, fetchOpPipeline) {
    ...
    if (result.resultSet !== undefined) {
      ...
      if (!fetchOpPipeline) {
        if (options.resultSet) {
          ...
          // for fetchAll, _getAllRows() get rows from buffered rows and
          // then perform as many round trips as are required to
          // complete the fetch
          result.rows = await resultSet._getAllRows();
          delete result.resultSet;
        }
      } else {
        await this._handleFetchInPipeline(result, resultSet, fetchOpPipeline);
      }
    }
    ...
    // are fetched
    await this._processImplicitResults(result, options, fetchOpPipeline);

    return (result);
  }

  async _processExecuteManyArgs(sql, bindsOrNumIters, userOptions) {
    let options = {};
    let binds = [];
    let numIters;
    ...
    errors.assertParamValue(typeof sql === 'string', 1);
    if (arguments.length == 3) {
      options = this._verifyExecOpts(userOptions, true);
    }
    this._addDefaultsToExecOpts(options);
    ...
    return {sql, numIters, binds, options, executeManyFlag: true};
  }

  async _executeMany() {
    const { sql, numIters, binds, options, executeManyFlag }
    = await this._processExecuteManyArgs.apply(this, arguments);

    const result = await this._impl.execute(sql, numIters, binds,
      options, executeManyFlag);

    return this._transformExecuteManyResult(result, options);
  }

  _transformExecuteManyResult(result, options) {
  ...
  //---------------------------------------------------------------------------
  async executeMany() {
    return await this._executeMany.apply(this, arguments);
  }
  ...
  // pdbName (READONLY)
  //
  // Property for identifying the container associated with connection.
  //---------------------------------------------------------------------------
  get pdbName() {
    return this._impl && this._impl.getPdbName();
  }

  //---------------------------------------------------------------------------
  // dbUniqueName (READONLY)
  //
  // Property for identifying the DB_UNIQUE_NAME of the Oracle Database.
  //---------------------------------------------------------------------------
  get dbUniqueName() {
    return this._impl && this._impl.getDbUniqueName();
  }
  //---------------------------------------------------------------------------
  // hostName (READONLY)
  //
  ...
  //
  // Resume an existing sessionless transaction using given transactionId
  //---------------------------------------------------------------------------
  ...
  //---------------------------------------------------------------------------
  async directPathLoad(schema, table, columns, data) {
    errors.assertArgCount(arguments, 4, 5);
    errors.assertParamValue(typeof schema === 'string', 1);
    errors.assertParamValue(typeof table === 'string', 2);
    errors.assertParamValue(Array.isArray(columns) && columns.length, 3);
    if (!Array.isArray(data) || !data.every(Array.isArray))
      errors.throwErr(errors.ERR_DPL_INVALID_ROW_DATA);
    errors.assert(this._impl, errors.ERR_INVALID_CONNECTION);
    return await this._impl.directPathLoad(schema, table, columns, data);
  }

  _createFetchTypeHandler(fetchLobs) {
    if (fetchLobs) {
      // Returning undefined → default behavior → LOB locators
      return undefined;
    }
    // Auto-fetch LOBs
    return (metadata) => {
      switch (metadata.dbType) {
        ...
        case types.DB_TYPE_NCLOB:
          return { type: types.DB_TYPE_VARCHAR };

        case types.DB_TYPE_BLOB:
          return { type: types.DB_TYPE_RAW };
  ...
  async _runPipelineOpWithoutPipelining(resultImpl) {
    const op = resultImpl.op;
    const opType = op.opType;
    let res = {};
    if (opType === 'commit') {
      res = await this._commit();
    } else if (opType === 'execute') {
      res = await this._execute(op.statement, op.parameters, op.options);
      if (res.rows) {
        res.rows = undefined;
      }
    } else if (opType === 'executeMany')
      res = await this._executeMany(op.statement, op.parameters, op.options);
    else if (['fetchOne', 'fetchMany', 'fetchAll'].includes(opType)) {

      // Force a ResultSet and then fetch rows according to the requested
      // fetch variant in non-pipeline flow.
      op.options.resultSet = true;
      op.options.fetchTypeHandler = this._createFetchTypeHandler(op.fetchLobs);
      let shouldClose = false;
      try {
        res = await this._execute(op.statement, op.parameters, op.options);
        if (opType === 'fetchOne') {
          const row = await res.resultSet._getRow();
          res.rows = row ? [row] : [];
          shouldClose = true;
        } else if (opType === 'fetchMany') {
          if (op.numRows === 0) {
            res.rows = [];
          } else {
            const maxRows = res.resultSet._impl.maxRows;
            const rowsToFetch = (maxRows > 0) ?
              Math.min(op.numRows, maxRows) :
              op.numRows;
            res.rows = await res.resultSet._doGetRows(rowsToFetch);
          }
          shouldClose = true;
        } else if (opType === 'fetchAll') {
          res.rows = await res.resultSet._getAllRows();
        }
      } finally {
        if (shouldClose)
          await res.resultSet._close();
        delete res.resultSet;
      }
    } else
      errors.throwErr(errors.ERR_PIPELINE_OPERATION_NOT_SUPPORTED, opType);
    resultImpl.result = res;
  }

  async _runPipelineWithoutPipelining(resultsImpl, continueOnError) {
    const savedCallTimeout = this.callTimeout;
    try {

      // For consistency with protocol-level pipelining, do not enforce
      // call timeout while executing the fallback non-pipelined flow.
      if (savedCallTimeout > 0)
        this.callTimeout = 0;
      for (const resultImpl of resultsImpl) {
        try {
          await this._runPipelineOpWithoutPipelining(resultImpl);
        } catch (err) {
          if (!continueOnError)
            throw (err);
      ...
      return this._buildPipelineResults(resultsImpl);
    } finally {
      if (savedCallTimeout > 0)
        this.callTimeout = savedCallTimeout;
    }
  }

  _buildPipelineResults(resultsImpl) {
    const results = [];
    for (let i = 0; i < resultsImpl.length; i++) {
      results[i] = resultsImpl[i].result;
      if (resultsImpl[i].error) {
        if (!results[i]) results[i] = {};
        results[i].error = resultsImpl[i].error;
      }
    }
    return results;
  }
  ...
  // runPipeline()
  //
  // It runs the list of operations and returns an result array.
  //---------------------------------------------------------------------------
  async runPipeline(pipeline, continueOnError = false) {
    errors.assertArgCount(arguments, 1, 2);
    errors.assertParamValue(typeof continueOnError === 'boolean', 2);
    errors.assertParamValue(pipeline?.constructor?.name === 'Pipeline', 1);
    // validate connection state
    errors.assert(this._impl, errors.ERR_INVALID_CONNECTION);
    const results = [];
    for (const op of pipeline.operations) {
      // sample result (will be filled with corresponding operation result,
      // like rows, rowsAffected, metadata, error)
      results.push({op, result: {}});
    }

    // Use protocol-level pipelining only when the server supports it and
    // when there is more than one operation to benefit from batching.
    if (this._impl.supportsPipelining() && pipeline.operations.length > 1)
      return await this._impl.runPipelineWithPipelining(this, results,
        continueOnError);
    else
      return await this._runPipelineWithoutPipelining(results,
        continueOnError);
  }
  ...
  // appContext()
  //
  // Add Application Context, keyValues identified by name, namespaceName.
  //---------------------------------------------------------------------------
  appContext(namespaceName, keyValues) {
    errors.assertArgCount([namespaceName, keyValues], 2, 2);
    errors.assertParamValue(typeof namespaceName === 'string', 1);
    if (namespaceName.length === 0) {
      errors.throwErr(errors.ERR_APP_CONTEXT_EMPTY_NAMESPACE);
    }

    // Passing null/undefined/[] currently leaves existing entries intact in
    // thick mode and triggers an error in thin mode.
    // It does not clear the context; use clearAppContext() explicitly
    // when you need to remove keys.
    if (keyValues === null || keyValues === undefined) {
      keyValues = [];
    }
    errors.assertParamValue(Array.isArray(keyValues), 2);

    for (const entry of keyValues) {
      errors.assertParamValue(nodbUtil.isObject(entry) && !Array.isArray(entry), 2);
      const entries = Object.entries(entry);
      const [entryKey, entryValue] = entries[0] ?? [];

      errors.assert(entries.length === 1 &&
        typeof entryKey === 'string' &&
        typeof entryValue === 'string',
      errors.ERR_APP_CONTEXT_INVALID_KEY_VALUE,
      typeof entryKey,
      typeof entryValue);

      const keyLength = Buffer.byteLength(entryKey);
      errors.assert(keyLength <= APP_CONTEXT_KEY_MAX_BYTES,
        errors.ERR_APP_CONTEXT_KEY_TOO_LONG,
        keyLength,
        APP_CONTEXT_KEY_MAX_BYTES);

      const valueLength = Buffer.byteLength(entryValue);
      errors.assert(valueLength <= APP_CONTEXT_VALUE_MAX_BYTES,
        errors.ERR_APP_CONTEXT_VALUE_TOO_LONG,
        valueLength,
        APP_CONTEXT_VALUE_MAX_BYTES);
    }

    this._impl.appContext(namespaceName, keyValues);
  }

  //---------------------------------------------------------------------------
  // clearAppContext()
  //
  // Clears the Application Context, keyValues identified by name.
  //---------------------------------------------------------------------------
  clearAppContext(namespaceName) {
    errors.assertArgCount([namespaceName], 1, 1);
    errors.assertParamValue(typeof namespaceName === 'string', 1);
    this._impl.clearAppContext(namespaceName);
  }
