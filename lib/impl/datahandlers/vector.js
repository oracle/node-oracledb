...
class VectorDecoder extends BaseBuffer {
  ...
  //---------------------------------------------------------------------------
  // _verifyVectorDataSize()
  //
  // Verifies that the VECTOR image contains enough bytes for the declared
  // element count before allocating a typed array.
  //---------------------------------------------------------------------------
  _verifyVectorDataSize(numElements, elementSize) {
    const numBytes = numElements * elementSize;
  ...
  decode() {
    ...
    let arrayType, elementSize, sparseValue;
    ...
    if (flags & constants.TNS_VECTOR_FLAG_SPARSE) {
      ...
      this._verifyVectorDataSize(numElements, 4);
    ...
    if (vectorFormat === constants.VECTOR_FORMAT_FLOAT32) {
      ...
      if (numElements % 8 !== 0)
        errors.throwErr(errors.ERR_UNEXPECTED_DATA, numElements);
      numElements = numElements / 8;
    } else {
      errors.throwErr(errors.ERR_VECTOR_FORMAT_NOT_SUPPORTED, vectorFormat);
    }
    this._verifyVectorDataSize(numElements, elementSize);
    const value = new arrayType(numElements);
