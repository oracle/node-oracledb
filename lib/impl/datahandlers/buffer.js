...
class BaseBuffer {
  ...
  readBool(fromDBObject) {
    ...
    if (fromDBObject) {
      ...
      // as a separate field afterwards.
      return (buf[0] === 1);
    }
  }
  ...
  //---------------------------------------------------------------------------
  writeOracleNumber(value, writeLength = true) {
    ...
    let pos = this.reserveBytes(numPairs + 1 + writeLength + appendSentinel);
