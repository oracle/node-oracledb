...
function validateAcceptDataUnits(sdu, tdu, largeSDU) {
  const maxSdu = largeSDU ? constants.NSPABSSDULN : constants.NSPMXSDULN;
  if (sdu < constants.NSPMNSDULN || sdu > maxSdu) {
    const reason = `Negotiated SDU ${sdu} is outside the valid range ` +
      `${constants.NSPMNSDULN} to ${maxSdu}`;
    errors.throwErr(errors.ERR_INVALID_PACKET, reason);
  }
  if (tdu < constants.NSPMNTDULN || tdu > constants.NSPMXTDULN) {
    const reason = `Negotiated TDU ${tdu} is outside the valid range ` +
      `${constants.NSPMNTDULN} to ${constants.NSPMXTDULN}`;
    errors.throwErr(errors.ERR_INVALID_PACKET, reason);
  }
}
/**
...
function AcceptPacket(packet, sAtts) {
  ...
  let negotiatedSDU = packet.buf.readUInt16BE(constants.NSPACSDU);
  let negotiatedTDU = packet.buf.readUInt16BE(constants.NSPACTDU);

  sAtts.largeSDU = false;

  if (sAtts.version >= 315) {
    /* Large SDU Support */
    negotiatedSDU = packet.buf.readUInt32BE(constants.NSPACLSD);
    negotiatedTDU = packet.buf.readUInt32BE(constants.NSPACLTD);
    sAtts.largeSDU = true;
  ...
  validateAcceptDataUnits(negotiatedSDU, negotiatedTDU, sAtts.largeSDU);
  sAtts.sdu = negotiatedSDU;
  sAtts.tdu = negotiatedTDU;
...
function MarkerPacket(isLargeSDU) {
  ...
  this.fromPacket = function(packet, nsi) {
    ...
    let expectedFlag = constants.NIQRMARK;
    ...
    switch (this.type) {
      ...
      case constants.NSPMKTD1:
        ...
        nsi.isBreak = true;
        if (nsi.pipelineEnabled) {
          expectedFlag = constants.NIQBMARK;
        }
        if (this.data == expectedFlag) {
          nsi.isReset = true;
        }
        break;
      default:
...
function ControlPacket() {
  ...
  this.fromPacket = function(packet) {
    ...
    let err1;
    let err2;
    if (packet.buf.length < constants.NSPCTLCMD + 2) {
      errors.throwErr(errors.ERR_INVALID_PACKET);
    }
    ...
    switch (this.cmd) {
      case constants.NSPCTL_SERR:
        if (packet.buf.length < constants.NSPCTLDAT + 12) {
          errors.throwErr(errors.ERR_INVALID_PACKET);
        }
        ...
        if (err1 == NSECMANSHUT) {
          ...
          this.errno = err1;
          this.notifLen = err2;
          const copyStart = constants.NSPCTLDAT + 12;
          const payloadAvailable = packet.buf.length - copyStart;
          const maxNotifBytes = this.sAtts && this.sAtts.largeSDU
            ? constants.NSPABSSDULN
            : constants.NSPMXSDULN;
          // Ensure notification length fits in the packet body and negotiated SDU to avoid over-allocation.
          if (err2 < 0 || err2 > payloadAvailable || err2 > maxNotifBytes) {
            errors.throwErr(
              errors.ERR_INVALID_PACKET,
              `Invalid notification length ${err2}`
            );
          }
          this.notif = Buffer.allocUnsafe(err2 + 1).fill(0);
          packet.buf.copy(this.notif, 0, copyStart, copyStart + err2);
        } else {
          this.errno = err1;
