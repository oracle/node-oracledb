...
async function getConnectionInfo(userConfig) {
  let connStr = await resolveConnectStr(userConfig.connectString, userConfig.configDir);
  let nvpair;
  ...
  let serverVal = findValue(nvpair, ["DESCRIPTION", "CONNECT_DATA", "SERVER"]);
  let connClass = findValue(nvpair, ["DESCRIPTION", "CONNECT_DATA", "POOL_CONNECTION_CLASS"]);
  let svcname =   findValue(nvpair, ["DESCRIPTION", "CONNECT_DATA", "SERVICE_NAME"]);
  let poolPurity = findValue(nvpair, ["DESCRIPTION", "CONNECT_DATA", "POOL_PURITY"]);

  return [serverVal, connClass, svcname, poolPurity ];
}
 ...
 * @returns AddressNode
 */
let nlParamParser =  new NLParamParser;
async function resolveConnectStr(connectString, configDir) {
  let connStr = connectString.trim();
  let resolvedVal = connStr;
  if ((connStr.indexOf(')') == -1) || (connStr.indexOf('(') != 0)) {
    try {
      /* Try through ezconnect */
      let ezcnObj = new EzConnect(connStr);
      resolvedVal = await ezcnObj.getResolvedUrl();
    } catch (err) {
      try {
        /* try tns alias */
        let namesFilePath = tnsnamesFilePath(configDir);
        let p = await nlParamParser.initializeNlpa(namesFilePath);
...
async function resolveAddress(connStr, configDir) {
  let connstr = await resolveConnectStr(connStr, configDir);
...
class NetworkSession {
  ...
  async getAddress(addressNode, userConfig) {
    ...
    let address = await addressNode.execute(userConfig);
    /* Prepare connection attributes */
    let uuid = this.sAtts ? this.sAtts.uuid : null;
  ...
  async connect2(address) {
    ...
    /* Read the response */
      ...
      if (packet.type === constants.NSPTRF) { /* REFUSE */
        ...
        let nvpair = createNVPair(this.refusePkt.dataBuf);
        this.refusePkt = null;
        ...
        let adrLen, adrStr, redirAddressNode, redirConnData;
        ...
        redirAddressNode = await resolveAddress(adrStr);
  ...
  */
  async connect(userConfig) {
    let connStr = userConfig.connectString ? userConfig.connectString : userConfig.connectionString;
    let addressNode = await resolveAddress(connStr, userConfig.configDir);
    let address;
    try {
      address = await this.getAddress(addressNode, userConfig);
    } catch (err) {
