...
// Maximum supported descriptor byte length.
const DEFAULT_MAX_DESCRIPTOR_LENGTH = 512 * 8;

// Maximum supported descriptor nesting depth before rejecting the parse.
const DEFAULT_MAX_DESCRIPTOR_DEPTH = 64;

function checkLength(nvString, maxLength) {
  if (Buffer.byteLength(nvString, 'utf8') > maxLength) {
    errors.throwErr(errors.ERR_CONNECT_DESCRIPTOR_LIMIT_EXCEEDED,
      'byte length', maxLength);
  }
}

function checkDepth(depth, maxDepth) {
  if (depth > maxDepth) {
    errors.throwErr(errors.ERR_CONNECT_DESCRIPTOR_LIMIT_EXCEEDED,
...
/**
  ...
  */
function createNVPair(nvString, options = {}) {
  const maxLength = options.maxLength ?? DEFAULT_MAX_DESCRIPTOR_LENGTH;
  const maxDepth = options.maxDepth ?? DEFAULT_MAX_DESCRIPTOR_DEPTH;
  checkLength(nvString, maxLength);
  const nvt = new NVTokens();
  nvt.parseTokens(nvString);
  return readTopLevelNVPair(nvt, 1, maxDepth);
}
...
/*
  ...
  * value: atom | NVList
  */
function readTopLevelNVPair(nvt, depth, maxDepth) {
  ...
  if ((tk = nvt.getToken()) == TKN_COMMA) {
    ...
    return readRightHandSide(nvp, nvt, depth, maxDepth);
  }
 ...
 *   value: atom | NVList
 */
function readNVPair(nvt, depth, maxDepth) {
  checkDepth(depth, maxDepth);
 ...
 * value: atom | NVList
*/
function readRightHandSide(nvp, nvt, depth, maxDepth) {
  let tk = nvt.getToken();
  switch (tk) {
    case TKN_EQUAL:
      nvt.eatToken();
      ...
      if (tk == TKN_LITERAL) {
        ...
        // NVPair.
        readNVList(nvt, nvp, depth, maxDepth);
      }
      break;
 ...
 */
function readNVList(nvt, parent, depth, maxDepth) {
  while (true) {
    ...
    const child = readNVPair(nvt, depth + 1, maxDepth);
