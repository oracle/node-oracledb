...
class ThinLobImpl extends LobImpl {
  ...
  // trim()
  //
  // Internal method for trimming the LOB.
  //---------------------------------------------------------------------------
  async trim(newSize) {
    this.checkConn();
    const options = {
      operation: constants.TNS_LOB_OP_TRIM,
      sourceLobImpl: this,
      sendAmount: true,
      amount: newSize
    };
  ...
  async write(offset, data) {
    ...
    await this._sendMessage(options);
    // Take the offset into account while setting the length
    // Note that offset is 1-based
    this._length = Math.max(this._length, (offset - 1) + data.length);
  }
