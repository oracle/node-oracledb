...
class ThinConnectionImpl extends ConnectionImpl {
  ...
  static _createNamespace(nameSpaceName) {
    return {
      nameSpaceName,
      keyValues: [],
      pendingClear: false
  ...
  static _mergeKeyValues(namespace, keyValues) {
    const mergedKeyValues = namespace.keyValues.slice();
    for (const entry of keyValues) {
      const entryKey = Object.keys(entry)[0];
      const existingIndex = mergedKeyValues.findIndex((existingEntry) =>
        Object.prototype.hasOwnProperty.call(existingEntry, entryKey));
      if (existingIndex !== -1) {
        mergedKeyValues.splice(existingIndex, 1);
      }
      mergedKeyValues.push(entry);
    }
    namespace.keyValues = mergedKeyValues;
    namespace.pendingClear = false;
  }
  ...
  async close() {
    try {
      this.clearEndUserSecurityContext();
  ...
  async _getExecuteMessage(statement, numIters, binds, options,
  ...
  async _execute(statement, numIters, binds, options, executeManyFlag, internalTempLobs = []) {
    ...
    const message = await this._getExecuteMessage(statement, numIters, binds,
      options, executeManyFlag, internalTempLobs);
    ...
    await this._handleDefines(statement, message);

    // process results
    return this._processExecuteResult(statement, numIters, options,
      executeManyFlag, message);
  }

  //---------------------------------------------------------------------------
  // _sendMessagesForPipeline()
  //
  // Sends the messages for the pipeline to the database for processing.
  // This is async function only because of await adapter.pauseWrite()
  // which is used to slow down writing to network socket.
  //---------------------------------------------------------------------------
  async _sendMessagesForPipeline(messages, continueOnError) {
    for (const messageObj of messages) {
      if (messageObj.pipelineResultImpl.error)
        continue;
      const message = messageObj.message;
      const opType = messageObj.pipelineResultImpl.op.opType;
      try {
        if (['execute', 'executeMany', 'fetchOne', 'fetchMany',
          'fetchAll'].includes(opType)) {
          const statement = messageObj.statement;
          if (statement.isPlSql && (statement.cursorId === 0 ||
            statement.requiresFullExecute)) {
            message.numExecs = 1;
            message.noImplicitRelease = true;
            await this._protocol._encodeMessage(message);
            message.requiresSingleExecute = true;
          } else if (message.numExecs > 0) {
            await this._protocol._encodeMessage(message);
          }
        } else if (opType === 'commit')
          await this._protocol._encodeMessage(message);
      } catch (err) {
        this._returnStatement(messageObj.statement);
        if (!continueOnError) {
          throw (err);
        }
        messageObj.pipelineResultImpl.error = err;
      }
    }
  }

  //---------------------------------------------------------------------------
  // _createMessagesForPipelineOp()
  //
  // Creates a single message for a pipeline operation.
  //---------------------------------------------------------------------------
  async _createMessagesForPipelineOp(conn, operation) {
    const opType = operation.opType;
    if (opType === 'commit') {
      errors.assert(conn._impl, errors.ERR_INVALID_CONNECTION);
      return { message: new messages.CommitMessage(this) };
    }

    let processedArgs;
    switch (opType) {
      case 'execute':
        processedArgs = await conn._processExecuteArgs(operation.statement,
          operation.parameters, operation.options);
        break;
      case 'executeMany':
        processedArgs = await conn._processExecuteManyArgs(operation.statement,
          operation.parameters, operation.options);
        break;
      case 'fetchOne':
      case 'fetchMany':
      case 'fetchAll': {
        const options = {
          ...operation.options,
          fetchTypeHandler: conn._createFetchTypeHandler(operation.fetchLobs)
        };
        processedArgs = await conn._processExecuteArgs(operation.statement,
      ...
      default:
        errors.throwErr(errors.ERR_PIPELINE_OPERATION_NOT_SUPPORTED, opType);
    }

    const {sql, numIters, binds, options, executeManyFlag} = processedArgs;
    return await this._prepareForExecute(sql, numIters, binds, options,
      executeManyFlag);
  }

  //---------------------------------------------------------------------------
  // _createMessagesForPipeline()
  //
  ...
  //---------------------------------------------------------------------------
  async _createMessagesForPipeline(conn, resultsImpl, continueOnError) {
    const messagesForPipeline = [];
    let tokenNum = 1;
    for (const resultImpl of resultsImpl) {
      let messageObj;
      try {
        messageObj = await this._createMessagesForPipelineOp(conn,
          resultImpl.op);
      } catch (err) {
        if (!continueOnError)
          throw (err);
        resultImpl.error = err;
        continue;
      }
      messageObj.message.pipelineMessage = true;
      messageObj.pipelineResultImpl = resultImpl;
      messageObj.message.tokenNum = tokenNum;
      tokenNum++;
      messagesForPipeline.push(messageObj);
    }
    return messagesForPipeline;
  }

  _processExecuteResult(statement, numIters, options, executeManyFlag,
    message) {
    const result = {};
    if (message.warning) {
  ...
  //---------------------------------------------------------------------------
  async _completePipelineOp(messageObj, conn) {
    const message = messageObj.message;
    const opType = messageObj.pipelineResultImpl.op.opType;

    // Post-processing of pipeline message happen once after reading response
    // of all the operations is completed in the first phase.
    await message.postProcess();
    await this._protocol._processCallStatus(message);
    ...
    if (['execute', 'executeMany', 'fetchOne', 'fetchMany',
      'fetchAll'].includes(opType)) {
      const statement = messageObj.statement;
      const numIters = messageObj.numIters;
      const options = messageObj.options;
      const executeManyFlag = messageObj.executeManyFlag;
      try {

        // for PL/SQL blocks that required a single execute, perform any
        // remaining executes now
        if (message.requiresSingleExecute) {
          statement.requiresFullExecute = false;
          message.numExecs = numIters - 1;
          message.offset = 1;
          message.noImplicitRelease = false;
          ...
          message.requiresSingleExecute = false;
        } else {
          statement.requiresFullExecute = false;
        }
        ...
        // process results
        messageObj.resultTemp = this._processExecuteResult(statement, numIters,
          options, executeManyFlag, message);
      } catch (err) {
        this._returnStatement(messageObj.statement);
        throw (err);
      } finally {
        for (const lob of messageObj.internalTempLobs) {
          this._tempLobsToClose.push(lob._locator);
          this._tempLobsTotalSize += lob._locator.length;
        }
      }
      if (opType === 'executeMany')
        await conn._transformExecuteManyResult(messageObj.resultTemp,
          messageObj.options);
      else if (opType === 'execute') {
        await conn._handleFetchForExecuteInPipeline(messageObj.resultTemp, messageObj.options);
      }

      if (['fetchOne', 'fetchMany', 'fetchAll'].includes(opType)) {
        // fetch rows
        await conn._transformExecuteResult(messageObj.resultTemp,
          messageObj.options, messageObj.pipelineResultImpl.op);
      }
    }
  }

  //---------------------------------------------------------------------------
  // _completePipelineOps()
  //
  ...
  //---------------------------------------------------------------------------
  async _completePipelineOps(messagesForPipeline, conn, continueOnError) {
    for (const messageObj of messagesForPipeline) {
      const resultImpl = messageObj.pipelineResultImpl;
      if (resultImpl.error)
        continue;
      try {
        await this._completePipelineOp(messageObj, conn);
        if (!messageObj.pipelineResultImpl.error) {
          messageObj.pipelineResultImpl.result = messageObj.resultTemp;
        }
      } catch (err) {
        this._returnStatement(messageObj.statement);
        if (!continueOnError) {
  ...
  // only supported with Oracle Database 23ai and later.
  supportsPipelining() {
    return this.nscon.pipeliningSupport;
  }

  //---------------------------------------------------------------------------
  // runPipelineWithPipelining()
  //
  // Run the pipeline with pipelining when the database supports it.
  //---------------------------------------------------------------------------
  async runPipelineWithPipelining(conn, resultsImpl, continueOnError) {
    this.pipelineMode = continueOnError ?
      constants.TNS_PIPELINE_MODE_CONTINUE_ON_ERROR :
      constants.TNS_PIPELINE_MODE_ABORT_ON_ERROR;
    this.nscon.pipelineEnabled = true;

    // Create all pipeline messages
    const messagesForPipeline = await this._createMessagesForPipeline(conn,
      resultsImpl, continueOnError);
    try {
      this._protocol.readBuf.resetPackets();

      // Encode all messages
      await this._sendMessagesForPipeline(messagesForPipeline,
        continueOnError);

      // endPipeline message
      const endMessage = new messages.EndPipelineMessage(this);

      // Decode responses for all operation messages, then process the final
      // end-pipeline response and cleanup any remaining response packets.
      await this._protocol.endPipeline(messagesForPipeline,
        endMessage, this, continueOnError);

      // Complete post processing of results (and deferred operations in
      // non-pipeline mode)
      await this._completePipelineOps(messagesForPipeline, conn,
        continueOnError);

      return conn._buildPipelineResults(resultsImpl);
    } finally {
      this.pipelineMode = 0;
      this.nscon.pipelineEnabled = false;
    }
  }

  //---------------------------------------------------------------------------
  // runPipelineWithoutPipelining()
  //
  ...
  // run with pipelining.
  //---------------------------------------------------------------------------
  async runPipelineWithoutPipelining(conn, resultsImpl, continueOnError) {
    return await conn._runPipelineWithoutPipelining(resultsImpl,
      continueOnError);
  }
  ...
  _postConnect(authMessage) {
    ...
    this.dbName = authMessage.sessionData['AUTH_SC_DBUNIQUE_NAME'];
    this.dbUniqueName = authMessage.sessionData['AUTH_SC_REAL_DBUNIQUE_NAME'];
    ...
    // Set pdbName only from Oracle Database version 12.2 onwards.
    if (this.serverVersion >= 1202000000) {
      this.pdbName = authMessage.sessionData.AUTH_DBNAME;
    }
  }
  ...
  _prepare(sql, options) {
    const forceNew = Boolean(options.fetchTypeMap);
    const statement = this._getStatement(sql, options.keepInStmtCache, forceNew);
    statement.bufferRowIndex = 0;
  ...
  async _bind(stmt, variable, pos = 0, internalTempLobs = []) {
    ...
    if (stmt.isPlSql && variable.maxSize > 32767) {
      if (this.pipelineMode)
        errors.throwErr(errors.ERR_PIPELINE_LARGE_BUFFERS_NOT_SUPPORTED);
  ...
  async _handleDefines(statement, message) {
  ...
  async _prepareForExecute(sql, numIters, binds, options, executeManyFlag) {
    const statement = this._prepare(sql, options);
    const internalTempLobs = [];
    const message = await this._getExecuteMessage(statement, numIters, binds,
      options, executeManyFlag, internalTempLobs);
    return {statement, numIters, options, executeManyFlag, internalTempLobs,
      message};
  }
  ...
  //---------------------------------------------------------------------------
  _getStatement(sql, cacheStatement = false, forceNew = false) {
    return this.statementCache.getStatement(sql, cacheStatement,
      forceNew || this._drcpEstablishSession);
  }
  ...
  //---------------------------------------------------------------------------
  // Returns the name of a container associated with the connection.
  //---------------------------------------------------------------------------
  getPdbName() {
    return this.pdbName;
  }
  ...
  //---------------------------------------------------------------------------
  // Returns the Oracle Database unique name associated with the connection.
  //---------------------------------------------------------------------------
  getDbUniqueName() {
  ...
  // Queue key/value pairs to be sent to the server.
  // If the namespace already has pending entries, merge and replace by key.
  appContext(namespaceName, keyValues) {
    const existingNamespace = this._nameSpaceMap.get(namespaceName);

    // If namespace doesn't exist or was cleared, just initialize it
    if (!existingNamespace || existingNamespace.pendingClear) {
      const ns = existingNamespace || ThinConnectionImpl._createNamespace(namespaceName);
      ns.pendingClear = false;
      ns.keyValues = keyValues;
      this._nameSpaceMap.set(namespaceName, ns);
      return;
    }

    ThinConnectionImpl._mergeKeyValues(existingNamespace, keyValues);
  }

  // This will clear all context on the server.
  // If the nameSpaceName exists , it updates else creates a new entry with clear flag set.
  clearAppContext(namespaceName) {
    const ns = ThinConnectionImpl._createNamespace(namespaceName);
    ns.pendingClear = true;
    this._nameSpaceMap.set(namespaceName, ns);
  }

  //---------------------------------------------------------------------------
  // directPathLoad()
  //   Implements direct path loading into a table.
  //---------------------------------------------------------------------------
  async directPathLoad(schemaName, tableName, columnNames, data) {
    const prepareMessage = new messages.DirectPathPrepareMessage(this);
    prepareMessage.schemaName = schemaName;
    prepareMessage.tableName = tableName;
    prepareMessage.columnNames = columnNames;
    await this._protocol._processMessage(prepareMessage);

    const opMessage = new messages.DirectPathOpMessage(this);
    opMessage.prepare(prepareMessage.cursorId, constants.TNS_DP_OP_ABORT);

    try {
      const loadMessage = new messages.DirectPathLoadStreamMessage(this);
      loadMessage.prepare(prepareMessage.cursorId, data, prepareMessage.columnMetadata);
      await this._protocol._processMessage(loadMessage);
      opMessage.opCode = constants.TNS_DP_OP_FINISH;
    } finally {
      await this._protocol._processMessage(opMessage);
    }
  }
  ...
  //
  // Set the End Security Context.
  //---------------------------------------------------------------------------
  setEndUserSecurityContext(context) {
    if (!this._protocol.caps.supportsEndUserSecurityContext) {
      errors.throwErr(errors.ERR_UNSUPPORTED_DEEP_DATA_SECURITY_FEATURE);
    }
    errors.assertParamValue(context !== null && context !== undefined, 1);

    if (this.securityContext && this.securityContext !== context) {
      this.securityContext.clearEncodedPayload();
    }
