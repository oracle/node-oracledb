...
class DbObjectPickleBuffer extends GrowableBuffer {
  ...
  readHeader(obj) {
    ...
    this.readLength();
    if (obj.imageFlags & constants.TNS_OBJ_IS_DEGENERATE) {
      errors.throwNotImplemented('DbObject attributes stored as a locator');
    }
...
class ThinDbObjectImpl extends DbObjectImpl {
    ...
    if (value === null || value === undefined) {
      ...
      switch (type) {
        ...
        case types.DB_TYPE_OBJECT:
          ...
          break;
        default:
          errors.throwNotImplemented(type);
      }
    }
  }
  ...
  _unpackValue(buf, type, typeClass) {
    ...
    const caps = this._objType._connection._protocol.caps;
    switch (type) {
      case types.DB_TYPE_NUMBER:
