...
class Protocol {
  ...
  async _processPipelineMessage(messageObj, connImpl, continueOnError) {
    let opProcessed = false;
    ...
    try {
      ...
      message.decode(this.readBuf);
      opProcessed = true;
      this._checkAndRaiseException(message);
    } catch (err) {
      if (!opProcessed && !this.connInProgress &&
          err.code !== errors.ERR_CONNECTION_CLOSED_CODE) {
        this.readBuf.resetPackets();
      }
      if (messageObj.statement)
        connImpl._returnStatement(messageObj.statement);
      if (!continueOnError) {
        err.opProcessed = opProcessed;
        throw (err);
      }
  ...
  _checkAndRaiseException(message) {
    if (message.errorOccurred) {
      this._raiseException(message);
    }
  }
  /**
   ...
   */
  async endPipeline(messagesForPipeline, endMessage, connImpl,
    continueOnError) {
    await this._encodeMessage(endMessage);

    // required for cleanup of response packets
    let numResponsesToDiscard = messagesForPipeline.length + 1;
    try {
      for (const messageObj of messagesForPipeline) {
        const opProcessed = await this._processPipelineMessage(
          messageObj, connImpl, continueOnError);
        if (opProcessed)
          numResponsesToDiscard--;
      }

      // Process the final message
      await this.readBuf.waitForPackets(true);
      endMessage.process(this.readBuf);
      this._checkAndRaiseException(endMessage);
      numResponsesToDiscard = 0;

    } catch (err) {

      // If an operation response was already consumed before failure, account
      // for it before discarding the remaining pipeline responses.
      if (err.opProcessed)
        numResponsesToDiscard--;
      await this.readBuf.discardPipelineResponses(numResponsesToDiscard);
      throw (err);
    }
  }
  ...
  async _recoverFromError(caughtErr, message) {
     ...
     */
    try {
      if (message.isPipeline) {
        // We just cleanup all pending response packets
        this.readBuf.resetPackets();
      } else {
        this.breakMessage();
        this._breakInProgress = false;
  ...
  async _processCallStatus(message) {
    if (message.flushOutBinds) {
      await this.flushOutBindMessage(message);
    }
    this.txnInProgress = Boolean(message.callStatus &
      constants.TNS_EOCS_FLAGS_TXN_IN_PROGRESS);
  ...
  async _processMessage(message) {
    ...
    try {
      ...
      if (traceEnabled) {
        ...
        if (traceContext.userContext?.traceParent) {
          // The traceParent is set from instrumentation module.
          message.connection.appContext('CLIENTCONTEXT',
            [{ ora$opentelem$tracectx: traceContext.userContext.traceParent }]);
        }
      }
    ...
    await this._processCallStatus(message);
