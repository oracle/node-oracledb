...
class ExecuteMessage extends MessageWithData {
  ...
  writeExecuteMessage(buf) {
    ...
    if (this.noImplicitRelease) {
      execFlags |= constants.TNS_EXEC_OPTION_NO_IMPL_REL;
    }
    ...
    if (stmt.requiresDefine) {
      ...
      execFlags |= constants.TNS_EXEC_OPTION_IMPLICIT_RESULTSET;
    ...
    if (this.arrayDmlRowCounts) {
      execFlags |= constants.TNS_EXEC_OPTION_DML_ROWCOUNTS;
    }
    ...
    buf.writeUB4(execFlags);                         // al8i4[9] DML row counts/implicit
  ...
  encode(buf) {
    ...
    if (this.currentRow === 0) {
      const stmt = this.statement;
      if (stmt.cursorId !== 0 && !stmt.requiresFullExecute && !this.parseOnly
        && !stmt.requiresDefine && !stmt.noPrefetch && !stmt.isDdl
        && !this.batchErrors) {
