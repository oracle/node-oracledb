...
class MessageWithData extends Message {
  ...
  processRowHeader(buf) {
    ...
    if (numBytes > 0) {
      ...
      this.getBitVector(buf, numBytes);
    }
    numBytes = buf.readUB4();
    if (numBytes > 0) {
  ...
  isDuplicateData(columnName) {
    if (this.bitVector === null || this.bitVector === undefined) {
      return false;
    }
    let byteNum = Math.floor(columnName / 8);
    let bitNum = columnName % 8;
    return (this.bitVector[byteNum] & (1 << bitNum)) === 0;
  }

  processRowData(buf) {
    let value;
    for (let [col, variable] of this.outVariables.entries()) {
      if (variable.isArray) {
        variable.numElementsInArray = buf.readUB4();
        let values = new Array(variable.numElementsInArray).fill(null);
  ...
  processIOVector(buf) {
    ...
    for (let i = 0; i < numBinds; i++) {              // bind directions
      let bindInfo = this.statement.bindInfoList[i];
  ...
  processReturnParameter(buf) {
    ...
    if (this.arrayDmlRowCounts) {
      let numRows = buf.readUB4();
      let rowCounts = this.options.dmlRowCounts = [];
      for (let i = 0; i < numRows; i++) {
        let rowCount = buf.readUB8();
  ...
  process(buf) {
    this.bitVector = this.savedBitVector;
    super.process(buf);
  }

  savePoint(buf) {
    this.savedBitVector = this.bitVector;
    super.savePoint(buf);
  }
  ...
  writeBindParamsRow(buf, params, pos) {
    ...
    for (const bindInfo of params) {
      if (bindInfo.isReturnBind)
        continue;
      let variable = bindInfo.bindVar;
      if (variable.isArray) {
        let numElements = variable.values.length;
  ...
  processImplicitResultSet(buf) {
    ...
    for (let i = 0; i < numResults; i++) {
      ...
      buf.skipBytes(numBytes);
      let childResultSet = this.createCursorFromDescribe(buf);
