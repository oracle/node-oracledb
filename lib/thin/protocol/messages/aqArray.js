...
class AqArrayMessage extends AqBaseMessage {
  ...
  processReturnParameter(buf) {
    ...
    for (let i = 0; i < numIters; i++) {
      ...
      const msgIdSize = buf.readUB4();      // size of msgIds info
      if (msgIdSize > 0)
        buf.skipUB1();                      // size of msgIds array
      if (this.operation === constants.TNS_AQ_ARRAY_ENQ) {
        for (let j = 0; j < this.propsImpls.length; j++) {
          this.propsImpls[j].msgId = this._processMsgId(buf);
        }
      } else {
        propsImpl.msgId = this._processMsgId(buf);
      }
