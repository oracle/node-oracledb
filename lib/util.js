...
module.exports.wrap_fns = wrap_fns;
...
async function checkToken(attrs) {
  let accessToken;
  if (typeof attrs.accessToken === 'function') {
    accessToken = await attrs.accessToken(false);
    if (!isTokenValid(accessToken)) {
      accessToken = await attrs.accessToken(true);
    }
  } else {
