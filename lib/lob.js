...
class Lob extends Duplex {
  ...
  // returns undefined if the LOB is closed.
  //---------------------------------------------------------------------------
  get chunkSize() {
    this._checkNotBfile();
    return this._impl?.getChunkSize();
  }
  ...
  async getData(offset, amount) {
    ...
    if (offset === undefined) {
      ...
      errors.assertParamValue(Number.isSafeInteger(offset) && offset > 0, 1);
    }
    if (amount === undefined) {
      amount = 0;
    } else {
      errors.assertParamValue(Number.isSafeInteger(amount) && amount > 0, 2);
    }
    errors.assert(this._impl, errors.ERR_INVALID_LOB);
  ...
  //---------------------------------------------------------------------------
  // trim()
  //  trims non-BFILE LOBs to a smaller size as specified by the parameter.
  //---------------------------------------------------------------------------
  async trim(newSize) {
    this._checkNotBfile();
    ...
    if (newSize === undefined) {
      ...
      errors.assertParamValue(Number.isInteger(newSize) && newSize >= 0 &&
        newSize < 2 ** 32, 1);
    }
    await this._impl.trim(newSize);
  }
  ...
  //---------------------------------------------------------------------------
  get pieceSize() {
    return this._impl?.getPieceSize();
  }

  set pieceSize(value) {
    errors.assertPropValue(Number.isSafeInteger(value) && value >= 0, "pieceSize");
    errors.assert(this._impl, errors.ERR_INVALID_LOB);
